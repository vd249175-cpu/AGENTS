...
def load_service_config(source: str | Path | None = None) -> ServiceConfig:
    ...
    data["port"] = int(os.getenv(env_port_key) or os.getenv("AGENT_PORT") or data.get("port", 8011))
...
def _is_assistant_message(node: dict[str, Any]) -> bool:
    role = str(node.get("role") or "").lower()
    kind = str(node.get("type") or node.get("message_type") or "").lower()
    name = str(node.get("__class__") or node.get("lc") or "").lower()
    return role == "assistant" or kind in {"ai", "ai_message", "aimessage", "aimessagechunk"} or "aimessage" in name


def _assistant_message_texts(data: Any) -> list[str]:
    found: list[str] = []

    def visit(node: Any) -> None:
        if isinstance(node, dict):
            if _is_assistant_message(node):
        ...
        try:
            async for chunk in app.state.runtime.astream(
                ...
                if chunk_type in {"messages", "updates"}:
                    texts = _assistant_message_texts(chunk_data)
                    if chunk_type == "messages":
                        message_parts.extend(texts)
                    elif texts:
                        latest_update_reply = texts[-1]
                    await observer.set_status(
                        "running",
                        phase=chunk_type,
            ...
            reply = latest_update_reply or "".join(message_parts).strip()
            return {"ok": True, "reply": reply, "chunks": collected_chunks, "agent": observer.state.snapshot()}
        except Exception as exc:
            await observer.report_error(
                {
                    "error_type": type(exc).__name__,
